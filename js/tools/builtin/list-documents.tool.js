...
(function (root, factory) {
  ...
  function updateLiveCard(card, _args, result = {}, elapsedMs = 0, ui) {
    ...
    const badge = card.querySelector('.tool-card-badge'); if (badge) { badge.className = `tool-card-badge ${success ? 'status-success' : 'status-error'}`; badge.innerHTML = success ? `${CHECK_SVG} <span>${count} doc${count === 1 ? '' : 's'} indexado${count === 1 ? '' : 's'} (${elapsedMs || 0}ms)</span>` : `${ERROR_SVG} <span>${result?.error || 'Error al consultar'}</span>`; }
